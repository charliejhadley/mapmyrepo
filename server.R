...
# Section 1 ---------------------------------------------------------------
  ...
  source("server_map-repo.R", local = TRUE)$value
...
### Section 2 --------------------------------------------------------------- 
  ...
  output$heatMapPlot <- renderPlot({
    ...
    heat_map <- ggplot(world_data) +
      ...
      theme(plot.title = element_text(
        ...
        colour = "#6F3092",
        face = "bold"
      )) +  # Set title font size to 14pts
      labs(title = paste("Mapped", input$metric),
           fill = element_blank())
  ...
  fetchFigshareData <- reactive({
    ...
    params <- list(
      search_for = paste("'", input$firstname, input$surname, "'"),
      limit = 1000
    )
    url <- paste0(base_url, "?", URLencode(paste0(
      names(params), "=", unlist(params), collapse = "&"
    )))
    
    
    # Make API request
    response <- GET(url, add_headers(Authorization = paste(
      "token", Sys.getenv("figshare_api_token")
    )))
    print(paste("first call", url))
    ...
    if (status_code(response) == 200) {
      ...
      article_ids <- as.character(articles_data$id)
      ...
      do_the_map <- article_ids %>%
        future_map_dfr(~ {
          article_id <- .x
          full_url_citation <- paste0(endpoint2, article_id)
          
          # print(full_url_citation)
          
          # Get the article citation data
          citation_response <- GET(full_url_citation,
          ...
          # # Check if the citation request was successful
          if (status_code(citation_response) == 200) {
            citation_data <- fromJSON(content(citation_response, "text", encoding = "UTF-8"),
                                      flatten = TRUE)
      ...
      return(do_the_map)  # Return the combined results with citations
    } else {
      warning(paste(
        "Failed to fetch data from Figshare API:",
  ...
  output$searchResults <- DT::renderDataTable({
    ...
    if (!is.null(fetched_df) &&
        nrow(fetched_df) > 0) {
      # Check if fetched_df is not NULL and has rows
      ...
      datatable(
        ...
        options = list(
          dom = 't',
          # Only display the table without any other controls
          paging = FALSE,
          # Disable paging
          searching = FALSE,
          # Disable searching
      ...
      datatable(
        data.frame(Citation = "No results found"),
        escape = FALSE,
        options = list(
          dom = 't',
          paging = FALSE,
